...
module.exports = async (req, res) => {
  ...
  try {
    if (method === 'GET' && seg(0) === 'auth' && seg(1) === 'challenge') {
    ...
    if (method === 'POST' && seg(0) === 'auth' && seg(1) === 'signin') {
      ...
      try {
        ...
        if (recovered.toLowerCase() !== address.toLowerCase()) return json(res, { error: 'Signature mismatch' }, 401);
    ...
    if (method === 'GET' && seg(0) === 'auth' && seg(1) === 'me') {
    ...
    if (method === 'GET' && (segments.length === 0 || seg(0) === '')) {
      return json(res, { status: 'Orbitjob backend running', seed: true });
    }
    if (method === 'POST' && seg(0) === 'tasks' && seg(2) === 'execute') {
      const id = seg(1);
      const idx = taskCache.findIndex(t => t.id === id);
      ...
      try {
        ...
        catch { throw new Error(`AI returned invalid JSON. Response: ${raw.substring(0, 200)}`); }
        ...
        task.result = { summary: r.summary, data: r.data };
        task.confidenceScore = conf;
        task.verificationStatus = verified ? 'VERIFIED' : 'REJECTED';
        agent.status = 'IDLE';
        agent.completedTasks += 1;
        if (task.contractTaskId) {
          const br = await mockSubmitExecution(task.contractTaskId, JSON.stringify(r), JSON.stringify(rt), conf, agent.id);
          task.verificationStatus = br.verificationStatus; task.txId = br.txId; task.blockNumber = br.blockNumber;
        }
      } catch (err) {
        task.status = 'FAILED'; task.executionTrace.completedAt = new Date().toISOString();
    ...
    if (method === 'GET' && segments.length === 2 && seg(0) === 'tasks') {
      const task = taskCache.find(t => t.id === seg(1) || t.contractTaskId === Number(seg(1)));
    ...
    if (method === 'POST' && seg(0) === 'tasks') {
      ...
      const ao = agents.find(a => a.id === assignedAgent) || agents[0];
      const nt = { id: `contract-${receipt.contractId}`, title, description, constraints: constraints || '', reward: parseFloat(reward) || 0, deadline: deadline || null, status: 'PENDING', createdAt: new Date().toISOString(), executionTrace: null, verificationStatus: 'NOT_VERIFIED', result: null, confidenceScore: null, assignedAgent: ao.id, contractTaskId: receipt.contractId, txId: receipt.txId, blockNumber: receipt.blockNumber };
      taskCache.push(nt);
      return json(res, nt, 201);
    }
    if (method === 'GET' && segments.length === 2 && seg(0) === 'agents') {
      const a = agents.find(x => x.id === seg(1));
      if (!a) return json(res, { error: 'Agent not found' }, 404);
      return json(res, a);
    }
    if (method === 'PUT' && segments.length === 2 && seg(0) === 'agents') {
      const i = agents.findIndex(x => x.id === seg(1));
    ...
    if (method === 'POST' && seg(0) === 'agents') {
      ...
      const na = { id: uuidv4(), name, model: model || 'qwen/qwen-2.5-coder:free', specialty: specialty || 'General', icon: icon || 'cpu', price: parseFloat(price) || 0, description, useCases: Array.isArray(useCases) ? useCases : [], rating: 0, completedTasks: 0, status: 'IDLE', accent: accentColors[agents.length % accentColors.length] };
      agents.push(na);
